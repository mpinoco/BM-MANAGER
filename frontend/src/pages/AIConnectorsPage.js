        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="financial" className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                    <h4 className="font-medium mb-2">Comparativa Anual</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span>Sin IA (pérdidas proyectadas):</span>
                        <span className="text-red-600 font-medium">
                          ${((fraudStats.lossWithoutAI || 0) * 12 / 1000000000).toFixed(2)}B CLP
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Con Gravit + Edgify:</span>
                        <span className="text-green-600 font-medium">
                          ${(((fraudStats.monthlySavings || 0) - (fraudStats.monthlySubscriptionCost || 0)) * 12 / 1000000000).toFixed(2)}B CLP
                        </span>
                      </div>
                      <hr />
                      <div className="flex justify-between font-bold">
                        ...
                        <span className="text-blue-600">
                          ${((fraudStats.netImpact || 0) * 12 / 1000000000).toFixed(2)}B CLP
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <Card className="p-6">
                <h3 className="text-lg font-semibold mb-4">Impacto Financiero por Proveedor</h3>
                <div className="space-y-4">
                  <div className="p-4 border-l-4 border-blue-500 bg-blue-50">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <p className="text-gray-600">Fraudes detectados</p>
                        <p className="font-bold">{Math.floor((fraudStats.totalEvents || 0) * 0.52)}</p>
                      </div>
                      <div>
                        <p className="text-gray-600">Ahorro generado</p>
                        <p className="font-bold text-green-600">
                          ${((fraudStats.totalSavings || 0) * 0.52 / 1000000).toFixed(1)}M
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-600">Precisión</p>
                        <p className="font-bold">98.3%</p>
                      </div>
                      <div>
                        <p className="text-gray-600">ROI individual</p>
                        <p className="font-bold text-blue-600">312%</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-4 border-l-4 border-purple-500 bg-purple-50">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <p className="text-gray-600">Fraudes detectados</p>
                        <p className="font-bold">{Math.floor((fraudStats.totalEvents || 0) * 0.48)}</p>
                      </div>
                      <div>
                        <p className="text-gray-600">Ahorro generado</p>
                        <p className="font-bold text-green-600">
                          ${((fraudStats.totalSavings || 0) * 0.48 / 1000000).toFixed(1)}M
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-600">Precisión</p>
                        <p className="font-bold">97.1%</p>
                      </div>
                      <div>
                        <p className="text-gray-600">ROI individual</p>
                        <p className="font-bold text-purple-600">258%</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 p-4 bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-lg">
                  ...
                  <p className="text-sm text-gray-700">
                    ...
                    con un ahorro neto proyectado de{' '}
                    <span className="font-bold text-green-600">
                      ${((fraudStats.netImpact || 0) * 12 / 1000000000).toFixed(2)}B CLP
                    </span> 
                    {' '}por año. La implementación dual Gravit + Edgify maximiza la cobertura y precisión.
                  </p>
                </div>
              </Card>
            </div>
            ...
            <Card className="p-6">
              <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-blue-600" />
                Tendencia Costo-Beneficio (Últimos {dateRange} días)
              </h3>
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={fraudStats.dailyTrends || []}>
                  ...
                  <XAxis dataKey="date" />
                  <YAxis />
                  <Tooltip formatter={(value) => [`$${value?.toLocaleString('es-CL')} CLP`, 'Valor']} />
                  <Line 
                    type="monotone" 
                    dataKey="value" 
                    stroke="#EF4444" 
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-sm text-red-700">Valor Total Fraudes</p>
                  <p className="text-xl font-bold text-red-800">
                    ${((fraudStats.dailyTrends || []).reduce((sum, day) => sum + (day.value || 0), 0) / 1000000).toFixed(1)}M
                  </p>
                </div>
                <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-sm text-green-700">Ahorro Generado</p>
                  <p className="text-xl font-bold text-green-800">
                    ${((fraudStats.dailyTrends || []).reduce((sum, day) => sum + (day.savings || 0), 0) / 1000000).toFixed(1)}M
                  </p>
                </div>
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-blue-700">Eficiencia IA</p>
                  <p className="text-xl font-bold text-blue-800">{fraudStats.preventionRate}%</p>
                </div>
              </div>
            </Card>
          </TabsContent>
